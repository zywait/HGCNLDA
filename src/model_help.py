...
class BaseModel(pl.LightningModule):
    ...
    def focal_loss(self, predict, label, alpha, gamma):
        predict = predict.reshape(-1)
        label = label.reshape(-1)
    ...
    def ce_loss(self, logits, label, pos_weight=None):
        if pos_weight is not None:
            weight = torch.Tensor([1.0, pos_weight]).to(label.device)
            loss = F.cross_entropy(input=logits, target=label, weight=pos_weight)
        else:
            loss = F.cross_entropy(input=logits, target=label)
        return {"loss_ce":loss,
                "loss":loss}

    def cep_loss(self, logits, label, pos_weight=None):
        if pos_weight is not None:
            loss = F.binary_cross_entropy_with_logits(input=logits, target=label, pos_weight=pos_weight)
        else:
    ...
    def imc_loss(self, predict, label, weight_decay):
        error = torch.square(predict-label)
        loss_basic = torch.sqrt(torch.mean(error))
        # dict = self.state_dict().copy()
        # weight3 = dict.get('decoder.weights.weight').clone()
        # loss_basic += weight_decay * (torch.sum(weight3**2)/2)
        return {"loss_imc": loss_basic,
            "loss": loss_basic}
